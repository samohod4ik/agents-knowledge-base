...
В этом публичном шаблоне документируйте *ваш* канонический remote как `<GIT_REMOTE>`. Не копируйте внутренние URL чужих хостов в заметки.
...
- `projects-data-verification` — только аудит: без автоисправления. По brief проверяет frontmatter, wikilink с путём, регистрацию skills, формулировки SCM и связанный инвентарь **вручную по файлам** (или опциональным локальным скриптом оператора, если он есть).
