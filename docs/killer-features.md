...
Всегда включённое правило Cursor (`obsidian-vault-maintenance-protocol.mdc`). Это **источник истины для агента**. Этот файл и остальные `docs/` — онбординг человека. После содержательного изменения кода, инфры, схемы или процесса обновите связанные заметки в том же ходе. Сведите 2a–2c и гейт `scm` с `.mdc`, не оставляйте более богатый текст только здесь.
