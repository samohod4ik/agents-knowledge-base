...
Общие командные правила Cursor (паттерн `rr77-cursor-rules`) — копии внутри `.cursor/rules/` каждого проекта. Хук `docs-before-commit`, если вы его возьмёте, только *напоминает*. Он не пишет документацию, не гоняет Graphify и не блокирует git.
