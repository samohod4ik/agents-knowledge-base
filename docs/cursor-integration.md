...
- [ ] AlwaysApply-стоп: `python <SKILLS_ROOT>/projects-data-verification/scripts/verify_projects_data.py` — категория `scm` clean. Полный `errors = 0` — цель журнала, не стоп задачи.
...
Не направляйте склонированные example-проекты на GitHub издателя, если вы не форкаете сам шаблон. `gh` для *этого* репозитория — GitHub.com. Issue/PR рабочих репо — на ваш `<GIT_REMOTE>`.
