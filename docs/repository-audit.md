...
Этот репозиторий = операционный Obsidian vault + Cursor rules/skills. Он **не** поставляет живые карточки чужой команды. Примеры в скелете — только `example_library` и `example_service`.
